...
class DeepConvNet:
    ...
    ネットワーク構成は下記の通り
        ...
        conv - relu - conv - relu - pool -
        conv - relu - conv - relu - pool -
        afine - relu - dropout - affine - dropout - softmax
    """
    def __init__(self, input_dim=(1, 28, 28),
         ...
         # 重みの初期化 ========
         # 各層のニューロン一つあたりが、前走のニューロンといくつの繋がりがあるか（TODO:自動で計算する）

         pre_node_nums = np.array([1*3*3, 16*3*3, 16*3*3, 32*3*3, 32*3*3, 64*3*3, 64*4*4, hidden_size])
         weight_init_scale = np.sqrt(2.0 / pre_node_nums) # ReLUを使う場合に推奨される初期値
